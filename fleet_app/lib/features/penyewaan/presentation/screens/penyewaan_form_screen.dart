...
class _PenyewaanFormScreenState extends State<PenyewaanFormScreen> {
  ...
  late final TextEditingController _kendaraanIdCtrl, _kodeCtrl, _masaSewaCtrl, _tglMulaiCtrl, _tglSelesaiCtrl, _penangCtrl, _lokasiCtrl, _salesCtrl, _nilaiCtrl;
  ...
  void _submit() {
    ...
    if (_isEdit) {
      context.read<PenyewaanBloc>().add(PenyewaanUpdateRequested(id: widget.existing!.id, kodePenyewa: _kodeCtrl.text, group: _group, masaSewa: int.parse(_masaSewaCtrl.text), tanggalMulai: mulai, tanggalSelesai: selesai, penanggungJawab: _penangCtrl.text, nilaiSewa: double.parse(_nilaiCtrl.text), lokasiSewa: _lokasiCtrl.text.isEmpty ? null : _lokasiCtrl.text, sales: _salesCtrl.text.isEmpty ? null : _salesCtrl.text));
    } else {
      context.read<PenyewaanBloc>().add(PenyewaanCreateRequested(kendaraanId: int.parse(_kendaraanIdCtrl.text), kodePenyewa: _kodeCtrl.text, group: _group, masaSewa: int.parse(_masaSewaCtrl.text), tanggalMulai: mulai, tanggalSelesai: selesai, penanggungJawab: _penangCtrl.text, nilaiSewa: double.parse(_nilaiCtrl.text), lokasiSewa: _lokasiCtrl.text.isEmpty ? null : _lokasiCtrl.text, sales: _salesCtrl.text.isEmpty ? null : _salesCtrl.text));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocListener<PenyewaanBloc, PenyewaanState>(
        listener: (ctx, state) {
          if (state is PenyewaanActionSuccess) { ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(content: Text(state.message), backgroundColor: AppTheme.success)); ctx.pop(); }
          else if (state is PenyewaanActionError) { ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(content: Text(state.failure.message), backgroundColor: AppTheme.error)); }
        },
          ...
          builder: (ctx, state) {
            ...
            return AppLoadingOverlay(isLoading: isLoading, child: SingleChildScrollView(
              ...
              child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                AppTextField(controller: _kodeCtrl, label: 'Kode Penyewa', prefixIcon: Icons.person_outlined, validator: (v) => _req(v, 'Kode Penyewa')),
                const SizedBox(height: 12),
                SwitchListTile(
                  value: _group, onChanged: (v) => setState(() => _group = v),
                  title: const Text('Penyewaan Group'), tileColor: AppTheme.surfaceVariant,
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                ),
                const SizedBox(height: 16),
                Row(children: [
                  Expanded(child: AppTextField(controller: _tglMulaiCtrl, label: 'Tanggal Mulai', prefixIcon: Icons.calendar_today_outlined, readOnly: true, onTap: () => _pickDate(_tglMulaiCtrl, (d) { _tglMulai = d; _calcMasaSewa(); }), validator: (v) => _req(v, 'Tanggal Mulai'))),
                  const SizedBox(width: 12),
                  Expanded(child: AppTextField(controller: _tglSelesaiCtrl, label: 'Tanggal Selesai', prefixIcon: Icons.event_outlined, readOnly: true, onTap: () => _pickDate(_tglSelesaiCtrl, (d) { _tglSelesai = d; _calcMasaSewa(); }), validator: (v) => _req(v, 'Tanggal Selesai'))),
                ]),
                const SizedBox(height: 16),
                AppTextField(controller: _masaSewaCtrl, label: 'Masa Sewa (hari)', keyboardType: TextInputType.number, prefixIcon: Icons.timer_outlined, validator: (v) => _num(v, 'Masa Sewa')),
                const SizedBox(height: 16),
                AppTextField(controller: _penangCtrl, label: 'Penanggung Jawab', prefixIcon: Icons.badge_outlined, validator: (v) => _req(v, 'Penanggung Jawab')),
                const SizedBox(height: 16),
                AppTextField(controller: _nilaiCtrl, label: 'Nilai Sewa (Rp)', keyboardType: TextInputType.number, prefixIcon: Icons.attach_money, validator: (v) => _num(v, 'Nilai Sewa')),
                const SizedBox(height: 16),
                AppTextField(controller: _lokasiCtrl, label: 'Lokasi Sewa (Opsional)', prefixIcon: Icons.location_on_outlined),
                const SizedBox(height: 16),
                AppTextField(controller: _salesCtrl, label: 'Sales (Opsional)', prefixIcon: Icons.person_outline),
