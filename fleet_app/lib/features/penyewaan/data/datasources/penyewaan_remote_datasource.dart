...
class PenyewaanRemoteDataSourceImpl implements PenyewaanRemoteDataSource {
  ...
  Future<({List<PenyewaanModel> items, PaginationMeta meta})> getAll({int page=1, int? kendaraanId, String? search, bool? aktif}) async {
    ...
    return (items: (data['data'] as List).map((e) => PenyewaanModel.fromJson(e)).toList(), meta: PaginationMeta.fromJson(data));
  }

  @override Future<PenyewaanModel> getById(int id) async { final r = await _apiClient.get('${ApiConstants.penyewaan}/$id'); return PenyewaanModel.fromJson(r.data['data']); }
  @override Future<PenyewaanModel> create(Map<String, dynamic> data) async { final r = await _apiClient.post(ApiConstants.penyewaan, data: data); return PenyewaanModel.fromJson(r.data['data']); }
  @override Future<PenyewaanModel> update(int id, Map<String, dynamic> data) async { final r = await _apiClient.put('${ApiConstants.penyewaan}/$id', data: data); return PenyewaanModel.fromJson(r.data['data']); }
  @override Future<void> delete(int id) async { await _apiClient.delete('${ApiConstants.penyewaan}/$id'); }
}
