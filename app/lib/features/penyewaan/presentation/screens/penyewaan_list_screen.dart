    ...
    return PopScope(
      ...
      child: Scaffold(
          ...
          actions: [
            Padding(
              ...
              child: PopupMenuButton<String>(
                ...
                onSelected: (v) {
                  setState(
                      () => _filterAktif = v == 'semua' ? null : v == 'aktif');
                  _reload();
                },
              ),
            ),
              ...
              if (state is PenyewaanLoaded) {
                final filteredItems = _filterAktif == null
                    ...
                    : state.items.where((item) {
                        final mulai =
                            DateTime.tryParse(item.tanggalMulai);
                        final selesai =
                            DateTime.tryParse(item.tanggalSelesai);
                      ...
                      SliverPadding(
                        ...
                        sliver: SliverList(
                          delegate: SliverChildBuilderDelegate(
                                          ...
                                          IconButton(
                                            ...
                                            onPressed: () async {
                                              final ok = await showConfirmDialog(ctx,
                                                  ...
                                                  message: 'Hapus penyewaan ${item.kodePenyewa}?');
                                              if (ok && ctx.mounted)
                                                ctx.read<PenyewaanBloc>().add(
                                                    PenyewaanDeleteRequested(item.id));
                                            },
                                            padding: const EdgeInsets.all(6),
                                            constraints: const BoxConstraints()),
                                        ],
                            ...
                            childCount: filteredItems.length * 2 - 1,
                          ),
                        ),
                      ),
                    ],
        ...
        floatingActionButton: FloatingActionButton.extended(
          onPressed: () async {
            await context.push('/penyewaan/create',
